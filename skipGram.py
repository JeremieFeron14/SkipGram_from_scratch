...
class SkipGram:
    def __init__(self, sentences, nEmbed=200, negativeRate=5, winSize=5, minCount=5, n_gram=3,
                 mode_OOV=False, alpha=0.75):
    ...
    def initialise(self):
        ...
        self.W = np.random.normal(0, 1, (nb_unique_words, self.nEmbed))
        self.C = np.random.normal(0, 1, (nb_unique_words, self.nEmbed))
        ...
        if self.mode_OOV:
            ...
            self.W = np.random.normal(0, 1, (N_full, self.nEmbed))
            # C only includes words
            self.C = np.random.normal(0, 1, (N, self.nEmbed))
    ...
    def similarity(self, word1, word2, add_embed=True):
        """
        ...
        return expit(np.dot(word_vector1, word_vector2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', help='path to store/read model (when training/testing)',
                        required=True)
    parser.add_argument('--mode_OOV', help='with OOV ngram representations', default= False)
    ...
    if not opts.test:
        ...
        sg = SkipGram(sentences, mode_OOV=opts.mode_OOV)
        sg.train()
        sg.save(opts.model)
        ...
        for a, b, _ in pairs:
            ...
            print("{:.5f}".format(sg.similarity(a, b)))
